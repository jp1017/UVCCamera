...
//	LOGI("need %d of 32k URBs for transfer", num_urbs);
...
//        r = 0;
        if (UNLIKELY(r < 0)) {
			...
			if (UNLIKELY(i == 0)) {
                //20170607错误执行到这里
				usbi_dbg("first URB failed, easy peasy");
				LOGE("first URB failed, easy peasy");
				free_iso_urbs(tpriv);
				return r;
			}
			...
			usbi_dbg("reporting successful submission but waiting for %d "
				"discards before reporting error", i);
            LOGI("reporting successful submission but waiting for %d "
                             "discards before reporting error", i);
			return LIBUSB_SUCCESS;
		}
	}
...
//20170607submit_transfer返回-1导致失败
static int op_submit_transfer(struct usbi_transfer *itransfer) {
	struct libusb_transfer *transfer
		= USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);

//    LOGI("submit_transfer, android, type: %d", transfer->type);
    switch (transfer->type) {
...
//            LOGI("submit_transfer, LIBUSB_TRANSFER_TYPE_ISOCHRONOUS, return: %d");
