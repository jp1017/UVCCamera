...
public class AVWriter {
	...
	public void close(ClosedCallback callback) {
		Log.w(TAG,"**********AVWriter close");
		if(avcEncoder.isOpened()) {
			Log.w(TAG,"**********avcEncoder will close");
			avcEncoder.close();
		}

		if(aacEncoder.isOpened()) {
			Log.w(TAG,"**********aacEncoder will close");
			aacEncoder.close();
	...
	private static void muxAudioVideo(String outPutFile, String videoFile, String audioFile, ClosedCallback callback) {
		...
		try {
			Log.w(TAG,"**********muxAudioVideo,bWriteEnd="+bWriteEnd);
			Movie movie = MovieCreator.build(videoFile);
			File file=new File(audioFile);
			if(file.renameTo(file)){
			}else{
				Log.w(TAG,"**********aacEncoder not opened");
				Thread.sleep(2000);
			}
			...
			Log.w(TAG,"**********88888888muxAudioVideo write end,bWriteEnd="+bWriteEnd);
