...
public class RtspClient {
  ...
  private static final String USER = "INST ISU";
...
//        path = "/" + matcher.group(4) + "/" + matcher.group(6);
    ...
    Log.w(TAG, "host: " + host + ", port: " + port + ", path: " + path);
  }
  ...
  private String addHeaders(String authorization) {
    return "CSeq: "
            ...
            + "User-Agent: INST ISU PUSHER"
            + "\r\n"

            + (!TextUtils.isEmpty(sessionId) ? "Session: " + sessionId + "\r\n" : "")
            // For some reason you may have to remove last "\r\n" in the next line to make the RTSP client work with your wowza server :/
            + (!TextUtils.isEmpty(authorization) ? "Authorization: " + authorization + "\r\n" : "")
            + "\r\n";
  ...
  private String sendAnnounceWithAuth(String authResponse) {
    ...
    Log.w("authorization: ", authorization);
