...
public class CameraClient implements ICameraClient {
	...
	public void select(final UsbDevice device) {
		if (DEBUG) KLog.w(TAG, "select:device=" + (device != null ? device.getDeviceName() : null));
		mUsbDevice = device;
	...
	@Override
	public void release() {
		if (DEBUG) KLog.w(TAG, "release:" + this);
		mUsbDevice = null;
	...
	public void removeSurface(final Surface surface) {
		if (DEBUG) KLog.w(TAG, "removeSurface:surface=" + surface + ",hash=" + surface.hashCode());
		final CameraHandler handler = mWeakHandler.get();
	...
	public void startPush() {
		KLog.w(TAG, "startPush: " + Thread.currentThread().getName());

		final CameraHandler handler = mWeakHandler.get();
		if ((handler != null)) {
			handler.sendEmptyMessage(MSG_PUSH_START);
		}
	}

	@Override
	public void stopPush() {
		KLog.w(TAG, "stopPush: " + Thread.currentThread().getName());

		final CameraHandler handler = mWeakHandler.get();
		if ((handler != null)) {
			handler.sendEmptyMessage(MSG_PUSH_STOP);
		}
	}

	protected boolean doBindService() {
		if (DEBUG) KLog.w(TAG, "doBindService:");
	...
	protected void doUnBindService() {
		if (DEBUG) KLog.w(TAG, "doUnBindService:");
		synchronized (mServiceSync) {
			if (mService != null) {
				final Context context = mWeakContext.get();
				if (context != null) {
	...
	private final ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(final ComponentName name, final IBinder service) {
			if (DEBUG) KLog.w(TAG, "onServiceConnected:name=" + name);
			synchronized (mServiceSync) {
		...
		public void onServiceDisconnected(final ComponentName name) {
			if (DEBUG) KLog.w(TAG, "onServiceDisconnected:name=" + name);
			synchronized (mServiceSync) {
				mService = null;
				mServiceSync.notifyAll();
			}
		...
		synchronized (mServiceSync) {
			if (mService == null) {
				try {
					...
					if (DEBUG) KLog.e(TAG, "getService:", e);
				}
			}
		}
		return mService;
	...
	private static final class CameraHandler extends Handler {
		...
		public boolean isRecording() {
			...
			try {
				...
				if (DEBUG) KLog.e(TAG, "isRecording:", e);
			}
			return false;
		}
		...
		private static final class CameraTask extends IUVCServiceCallback.Stub implements Runnable {
			...
			@Override
			public void run() {
				if (DEBUG) KLog.w(TAG_CAMERA, "run:");
				Looper.prepare();
				synchronized (mSync) {
				...
				Looper.loop();
				if (DEBUG) KLog.w(TAG_CAMERA, "run:finishing");
				synchronized (mSync) {
					mHandler = null;
					mParent = null;
					mSync.notifyAll();
...
// callbacks from service
			...
			public void onConnected() throws RemoteException {
				...
				if (DEBUG) KLog.w(TAG_CAMERA, "onConnected:");
				mIsConnected = true;
				if (mParent != null) {
			...
			public void onDisConnected() throws RemoteException {
				if (DEBUG) KLog.w(TAG_CAMERA, "onDisConnected:");
				mIsConnected = false;
				if (mParent != null) {
					if (mParent.mListener != null) {
...
//================================================================================
			public void handleSelect(final UsbDevice device) {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleSelect:");
				final IUVCService service = mParent.getService();
				if (service != null) {
					try {
						mServiceId = service.select(device, this);
					} catch (final RemoteException e) {
						if (DEBUG) KLog.e(TAG_CAMERA, "select:", e);
					}
				}
			}

			public void handleRelease() {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleRelease:");
				mIsConnected = false;
				mParent.doUnBindService();
			}

			public void handleConnect() {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleConnect:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (DEBUG) KLog.e(TAG_CAMERA, "handleConnect:", e);
				}
			}

			public void handleDisconnect() {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleDisconnect:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (DEBUG) KLog.e(TAG_CAMERA, "handleDisconnect:", e);
				}
				mIsConnected = false;
			}

			public void handleAddSurface(final Surface surface, final boolean isRecordable) {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleAddSurface:surface=" + surface + ",hash=" + surface.hashCode());
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.addSurface(mServiceId, surface.hashCode(), surface, isRecordable);
				} catch (final RemoteException e) {
					if (DEBUG) KLog.e(TAG_CAMERA, "handleAddSurface:", e);
				}
			}

			public void handleRemoveSurface(final Surface surface) {
				if (DEBUG) KLog.w(TAG_CAMERA, "handleRemoveSurface:surface=" + surface + ",hash=" + surface.hashCode());
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
			...
			public void handleStartRecording() {
				if (DEBUG) KLog.w(TAG, "handleStartRecording:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					...
					if (DEBUG) KLog.e(TAG_CAMERA, "handleStartRecording:", e);
				}
			}

			public void handleStopRecording() {
				if (DEBUG) KLog.w(TAG, "handleStopRecording:");
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					if (service.isRecording(mServiceId)) {
					...
					if (DEBUG) KLog.e(TAG_CAMERA, "handleStopRecording:", e);
				}
			}

			public void handleCaptureStill(final String path) {
				if (DEBUG) KLog.w(TAG, "handleCaptureStill:" + path);
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.captureStillImage(mServiceId, path);
				} catch (final RemoteException e) {
					if (DEBUG) KLog.e(TAG_CAMERA, "handleCaptureStill:", e);
				}
			}
			
			public void handleResize(final int width, final int height) {
				if (DEBUG) KLog.w(TAG, String.format("handleResize(%d,%d)", width, height));
				final IUVCService service = mParent.getService();
				if (service != null)
				try {
					service.resize(mServiceId, width, height);
				} catch (final RemoteException e) {
					if (DEBUG) KLog.e(TAG_CAMERA, "handleResize:", e);
				}
			}
			...
			private void handleStartPush(String videoIp) {
				...
				if (service != null)
					try {
						service.startPush(mServiceId);
					} catch (final RemoteException e) {
						KLog.e(TAG_CAMERA, "handleStartPush:", e);
					}
			}

			private void handleStopPush(final String path) {
				KLog.w(TAG_CAMERA, "handleStopPush:" + path);
				final IUVCService service = mParent.getService();
				if (service != null)
					try {
						service.stopPush(mServiceId);
					} catch (final RemoteException e) {
						KLog.e(TAG_CAMERA, "handleStopPush:", e);
					}
			}
