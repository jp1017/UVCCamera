...
public class MediaMuxerWrapper {
	 ...
	 * @throws IOException
	 */
	public MediaMuxerWrapper(Context context, String ext) throws IOException {
	...
	public void prepare() throws IOException {
		if (mVideoEncoder != null)
			mVideoEncoder.prepare(mContext);
		if (mAudioEncoder != null)
			mAudioEncoder.prepare(mContext);
	}
...
//**********************************************************************
	...
	/*package*/ synchronized boolean start() {
		if (DEBUG) Log.w(TAG,  "start:");
		mStatredCount++;
		if ((mEncoderCount > 0) && (mStatredCount == mEncoderCount)) {
			mMediaMuxer.start();
			mIsStarted = true;
			notifyAll();
			if (DEBUG) Log.w(TAG,  "MediaMuxer started:");
		}
		return mIsStarted;
	}
	...
	*/
	/*package*/ synchronized void stop() {
		if (DEBUG) Log.w(TAG,  "stop:mStatredCount=" + mStatredCount);
		mStatredCount--;
	...
	/*package*/ synchronized int addTrack(final MediaFormat format) {
		...
		if (DEBUG) Log.w(TAG, "addTrack:trackNum=" + mEncoderCount + ",trackIx=" + trackIx + ",format=" + format);
		return trackIx;
	}
