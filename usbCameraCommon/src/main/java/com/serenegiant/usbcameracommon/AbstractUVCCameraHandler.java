...
public abstract class AbstractUVCCameraHandler extends Handler {
	...
	public void close() {
		if (DEBUG) KLog.w(TAG, "close:");
		if (isOpened()) {
			stopPreview();
			sendEmptyMessage(MSG_CLOSE);
		}
		if (DEBUG) KLog.w(TAG, "close:finished");
	}
	...
	public void stopPreview() {
		if (DEBUG) KLog.w(TAG, "stopPreview:");
		...
		if (DEBUG) KLog.w(TAG, "stopPreview:finished");
	}
	...
	public static final class CameraThread extends Thread {
		...
		protected void finalize() throws Throwable {
			KLog.w(TAG, "CameraThread#finalize");
			super.finalize();
		}
		...
		public void handleOpen(final USBMonitor.UsbControlBlock ctrlBlock) {
			...
			if (DEBUG) KLog.w(TAG, "supportedSize:" + (mUVCCamera != null ? mUVCCamera.getSupportedSize() : null));
		}
		...
		public void handleStartPreview(final Object surface) {
			...
			try {
				if (DEBUG) KLog.w(TAG_THREAD, "handleStartPreview: mode: " + mPreviewMode);
				mUVCCamera.setPreviewSize(mWidth, mHeight, 1, 30, mPreviewMode, mBandwidthFactor);
			} catch (final IllegalArgumentException e) {
				try {
					if (DEBUG) KLog.w(TAG_THREAD, "handleStartPreview: fallback to YUV mode - 0");
...
//			mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_NV21);
		...
		public void handleStopPreview() {
			...
			if (DEBUG) KLog.w(TAG_THREAD, "handleStopPreview:finished");
		}
		...
		public void handleStartRecording() {
			...
			try {
				...
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(mWeakParent.get(), ".mp4");	// if you record audio only, ".m4a" is also OK.
...
//					new MediaAudioEncoder(muxer, mMediaEncoderListener);
		...
		public void handleRelease() {
			if (DEBUG) KLog.w(TAG_THREAD, "handleRelease:mIsRecording=" + mIsRecording);
			handleClose();
			mCallbacks.clear();
			if (!mIsRecording) {
			...
			if (DEBUG) KLog.w(TAG_THREAD, "handleRelease:finished");
		}
